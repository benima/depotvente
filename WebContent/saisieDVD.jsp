...
<form action="${saisieDVDURL}" method="post">
	<p>DVD Titre :<input name="titre" type="text" value="${param.titre}" />${erreurs["titre"]}</p>
	<p>DVD Auteur :<input name="auteur" type="text" value="${param.auteur}"/>${erreurs["auteur"]}</p>
	<br>
	<p>
	<label>Genre / Type : </label><br>
	<select name="type">
		...
		<option value="filmRomantique" ${filmRomantique}>Films Sentimental/Romantique</option>
		<option value="filmMusical" ${filmAction}>Films Music</option>
		<option value="filmGuerre" ${filmAction}>Films Guerre</option>
		<option value="filmHistorique" ${filmHistorique}>Films Historique</option>
		<option value="concert" ${filmAction}>Concert / Clips Videos</option>
		<option value="reportage" ${reportage}>Reportage</option>
	</select>
	</p>
	<br>
	<label>Date d'achat :</label><br>
	<input type="date" name="dateAchat" id="dateAchat"><br>${erreurs["dateAchat"]}
